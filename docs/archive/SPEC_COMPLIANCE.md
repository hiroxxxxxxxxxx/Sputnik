...
| **4-2** Effective Level × スロットル | Cockpit が `get_flight_controller_signal().throttle_level` の 0/1/2 を `_level_to_mode` で Boost/Cruise/Emergency に変換。`cockpit/mode.py` で定数定義 | 対応表どおり |
