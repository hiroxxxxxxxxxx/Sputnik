...
- **許可**
  ...
  - エントリポイント（scripts）から `with_ib_market_data_service` / `with_ib_connection` を介した利用
- **禁止**
...
## 8.1 avionics.ib の層構成（現行）

`avionics/ib` は責務ごとに以下へ分離する。

| 層 | 役割 | 代表モジュール |
|----|------|----------------|
| **services** | ユースケース公開境界。外部（scripts/他パッケージ）は原則ここだけ参照 | `market_data_service.py`, `whatif_order_service.py`, `schedule_service.py`, `healthcheck_service.py` |
| **clients** | IB API I/O とレスポンス取得 | `market_client.py`, `account_client.py`, `whatif_order_client.py`, `schedule_client.py` |
| **models** | IB 向け DTO/契約定義 | `contracts.py`, `fetch_results.py`, `schedule.py` |
| **infra** | 接続ライフサイクル等の補助 | `session.py` |

補足:
- `schedule_rules.py` は廃止済み。スケジュール判定ロジックは `schedule_service.py` に統合。
- `schedule_client.py` は「IB I/O + tradingHours パース」に限定し、通知判断は service 側で行う。

## 8.2 例外処理ポリシー（IB）

- **client 層**: I/O失敗・空レスポンス・必須フィールド欠落などの発生源で `ValueError` を送出する。
- **service 層**: client の `ValueError` を捕捉し、`symbol` / `account` / `as_of` / `ny_date` 等の業務文脈を付与して再送出する。
- **呼び出し側（scripts 等）**: service からの例外を最終的なユーザー向けメッセージに変換する。
...
```mermaid
flowchart TD
  batchStart[run_daily_signal_persist] --> withIb[with_ib_market_data_service]
...
- 実際の API 呼び出し（`reqHistoricalDataAsync` 等）は `avionics/ib/clients/*` 経由で実行される（オーケストレーションは `services/market_data_service.py`）。
...
- エントリポイント（scripts / bot）は `with_ib_market_data_service` で DataSource を得て `fc.refresh(...)` に注入する。
...
- **Telegram ボット**: `scripts/bot/telegram_cockpit_bot.py` は薄いエントリポイントで、実処理は `src/app/telegram/usecases.py` の `fetch_cockpit_report` などが担う。各コマンド（`/cockpit` 等）で `build_cockpit_stack` 後に `fc.refresh` を 1 回実行するため、**プロセス再起動なしでもコマンド単位で因子は毎回クールドスタート**に近い挙動になる。
