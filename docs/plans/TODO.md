...
| `target_futures` 設定手順 | CLI: `set_target_futures.py MNQ --main …` または `NQ …`（MGC/GC 可）。Telegram: `/settarget mnq …`。`/settarget` 許可は **TELEGRAM_TARGET_ADMIN_USER_IDS**（カンマ区切り user_id）またはプライベート運用で **TELEGRAM_CHAT_ID** を同一 user_id に。**数値は MNQ/MGC 相当枚数**（`/ping` で許可ID件数確認）。 |
