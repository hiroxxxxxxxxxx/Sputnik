...
## 7. U/S 復帰確認の DB 化（ステートフルカウンタ廃止）


| 対象ファイル                                | 内容                                                                                                     |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `src/avionics/factors/base_factor.py` | U/S 復帰確認で使っている `_target_level` / `_confirm_counter` / `_confirm_days_required` の stateful カウンタ実装を廃止する。 |
| `src/avionics/factors/u_factor.py`    | 復帰判定の根拠をメモリカウンタではなく、DBの連続日数レコード参照に切り替える。                                                               |
| `src/avionics/factors/s_factor.py`    | 復帰判定の根拠をメモリカウンタではなく、DBの連続日数レコード参照に切り替える。                                                               |
| `src/store/`（新規テーブル追加を含む）             | U/S 復帰確認用の連続日数・判定履歴を永続化し、日次バッチ再実行時も同一結果になるようにする。                                                       |


**背景**: 現状の U/S は更新回数依存の stateful カウンタで復帰確認を行うため、実行タイミングや再起動で挙動がぶれる余地がある。  
日次判定の再現性を担保するため、復帰確認の状態は DB で管理する。
