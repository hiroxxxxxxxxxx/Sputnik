...
## 15. 因子インスタンスの `level` / 復帰ヒステリシス状態の永続化


| 対象 | 内容 |
| --- | --- |
| 各 `*Factor` / `BaseFactor` | 再起動後も `level` および復帰確認まわりの内部状態を前回運用と整合させる。 |
| `store` / `FlightController` | `signal_daily` や専用スナップショットから因子状態を読み込み、組み立て時または初回 refresh 前に適用する。 |


**背景**: 運用高度（`altitude`）は DB の `state` から毎 refresh で解決するが、因子の `self.level` はプロセス内のみ。再起動直後の計器と実際のヒステリシス位置のずれを防ぐため、別タスクとして永続化・復元を設計する。

---

## 16. 運用高度（altitude）の DB 参照の責務整理


| 対象 | 内容 |
| --- | --- |
| `src/cockpit/stack.py` | `build_cockpit_stack` 組み立て時に `read_altitude_regime` を呼ぶ。 |
| `src/avionics/flight_controller.py` | `refresh(..., conn=...)` 経由で `store.state` を import して読む。 |
| スクリプト / `src/reports/fetch_reports.py` / `src/store/daily_signal.py` 等 | `conn` を渡して refresh する各所。 |


**整理したいこと**: `read_altitude_regime` の呼び出しを **オーケストレーション層（cockpit・スクリプト・store 呼び出し側）に寄せ**、`avionics` は **`altitude` を引数だけ受け取る**形にそろえる。組み立て時と refresh 時で DB を二重に読む可能性は、同一 `conn` ブロックで 1 回だけ読み、`build_cockpit_stack` と `refresh` に同じ値を渡す等の規約で整理する。

**完了の目安**: `grep read_altitude_regime` が `store` 定義と許容した境界モジュールに限定され、`avionics` に `store` の import が残らない（テスト用の `altitude=` 直渡しは維持）。
