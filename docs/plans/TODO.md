...
| `target_base_futures` 設定手順 | CLI: `set_target_futures.py MNQ --base …` または `NQ …`（MGC/GC 可）。Telegram: `/settarget mnq <base>`。`/settarget` 許可は **TELEGRAM_TARGET_ADMIN_USER_IDS**（カンマ区切り user_id）またはプライベート運用で **TELEGRAM_CHAT_ID** を同一 user_id に。**数値は MNQ/MGC 相当枚数**（`/ping` で許可ID件数確認）。 |
...
| 呼び出し側 | `scripts` / `scripts/telegram_cockpit_bot.py` / `store.daily_signal` / `Cockpit.pulse` で `read_altitude_regime(conn)` を呼び、`refresh(..., altitude=...)` に渡す。 |
...
| `scripts/telegram_cockpit_bot.py` | 既存の `store` API 経由方針を維持し、再度直結が入らないガードを設ける。 |
...
| `scripts/telegram_cockpit_bot.py` / `src/avionics/ib/fetcher.py` | baseline注入・whatIf取得失敗時の表示ポリシー（継続/失敗）を統一する。 |
