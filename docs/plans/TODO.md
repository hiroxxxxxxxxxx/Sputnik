...
- 10.1 targets.toml v2 編集ルール（運用メモ）
- 11. 補助計器の永続化（後フェーズA）
- 12. 承認フラグ永続化（後フェーズB）
- 13. ログ基盤（後フェーズC）
...
- 3. Engine actual ポジション取得（クローズ）
- 4. 高度（Altitude）の設定集約（クローズ）
- 7. U/S 復帰確認の DB 化（クローズ）
- 8. Cockpit 現在モードの起動時復元（方針変更でクローズ）
...
**結論**: クローズ。  
Engine actual は `FlightController` 経由で取得され、`account_parsers` で `EngineActualState` を構築して利用する方針に移行済み。
...
| 対象ファイル                                    | 内容                                                  |
| ----------------------------------------- | --------------------------------------------------- |
| `scripts/batch/run_v_knockin_monitor.py`（新規/拡張） | 1hごと実行される監視バッチで、コアタイム判定・2本目以降の足判定・当日監視ウィンドウ管理を実装する。 |
...
| 対象ファイル                   | 内容                                                                             |
| ------------------------ | ------------------------------------------------------------------------------ |
| 方針 | `current_mode` は DB 復元せず、各サイクル `FlightController.refresh` + IB 実ポジション + `target_futures` から再計算する。 |
| DB | `state.effective_level` は削除し、`mode` の `ap_mode` / `execution_lock` のみ復元する。 |
| 運用 | 再起動直後は最初の `pulse` が完了するまで計器結果を確定値として扱わない。 |


**背景**: `effective_level` の永続復元は二重の正（DB と実ポジション）を生む。  
再起動後の整合は DB 復元ではなく、毎サイクルの再計算で担保する。
...
| `target_base_futures` 復元 | 再起動時・ロール/リバランス時に運用開始基準（symbol別 base 目標先物枚数）を読み出して適用する。 |
| `target_base_futures` 設定手順 | Telegram: `/settarget mnq <base>`（MGC/GC/NQ も可）。`/settarget` 許可は **TELEGRAM_TARGET_ADMIN_USER_IDS**（カンマ区切り user_id）またはプライベート運用で **TELEGRAM_CHAT_ID** を同一 user_id に。**数値は MNQ/MGC 相当枚数**（`/ping` で許可ID件数確認）。 |
...
- `config/targets.toml` は **v2 正規化スキーマ**（`base` / `part_caps` / `overrides`）を使う。
- `base.<Mode>.<Part>`: mode×part の基本設定（`future`, `option_k1`, `option_k2`, `legs`）。
- `part_caps.<Part>`: part 固定制約。最終 legs は `base.legs AND part_caps` で合成される。
- `overrides.<Mode>.<Altitude>.<Part>`: 高度差分のみ記述（同値は書かない）。
- 高度依存の変更は `overrides` に限定し、`base` は高度非依存の既定値を保つ。
- `/target` は DB の base 値に加え、現在 altitude での有効 legs（PB/BPS/CC）を表示する。
...
| 呼び出し側 | `scripts` / `scripts/bot/telegram_cockpit_bot.py` / `store.daily_signal` / `Cockpit.pulse` で `read_altitude_regime(conn)` を呼び、`refresh(..., altitude=...)` に渡す。 |
...
## 17. DB接続責務の一本化（scripts/cockpit/telegram）

| 対象 | 内容 |
| --- | --- |
| `scripts/*` | `get_connection()` の直接利用を棚卸しし、`store` API 経由へ寄せる対象を段階定義する。 |
| `src/cockpit/cockpit.py` | `conn` 注入経路と `altitude` 外部注入経路の責務境界を明文化する。 |
| `scripts/bot/telegram_cockpit_bot.py` | 既存の `store` API 経由方針を維持し、再度直結が入らないガードを設ける。 |

**背景**: 現在は `telegram` で接続隠蔽を進めた一方、他 `scripts` には `get_connection()` 直接利用が残っている。  
方針を「scripts で接続管理してよい」か「store API のみ」に統一し、実装差分を解消する必要がある。

---

## 18. Cockpit `conn` 経路の存廃判断

| 対象 | 内容 |
| --- | --- |
| `src/cockpit/cockpit.py` | `pulse(connあり)` を正式運用として残すか、`altitude` 注入方式へ統一するかを決定する。 |
| `src/cockpit/stack.py` / `scripts/*` | 実運用フロー（FC直呼び vs Cockpit利用）に合わせて入口を一本化する。 |

**背景**: `Cockpit` は `conn` を受け取れるが、主要 scripts は `build_cockpit_stack` + `fc.refresh(...)` の経路を使うため、運用経路が二重化している。  
未使用経路を残すなら責務を明記し、不要なら縮退方針（deprecated）を定める。

---

## 19. S内訳のデータ可用性基準（`/breakdown`）

| 対象 | 内容 |
| --- | --- |
| `src/reports/format_breakdown_report.py` | `0.00` と `N/A` の意味を運用基準として明確化する（MM=0、whatIf不可、baseline欠損など）。 |
| `scripts/bot/telegram_cockpit_bot.py` / `src/avionics/ib/fetcher.py` | baseline注入・whatIf取得失敗時の表示ポリシー（継続/失敗）を統一する。 |

**背景**: 同じ U/S セクション内で `MM/NLV=0.00` と `S内訳=N/A` が併存し、原因切り分けが難しい。  
データ不足の種類ごとに表示ルールを定義し、運用判断を容易にする必要がある。
...
- 2026-03-31: 項目 3 をクローズ（Engine actual ポジション取得は `FlightController` 経由 + `account_parsers` での状態化へ移行）。
- 2026-03-31: 項目 17〜19 を追加（DB接続責務の一本化、Cockpit `conn` 経路の存廃判断、`/breakdown` S内訳の可用性基準を整理課題として明記）。
