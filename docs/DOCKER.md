...
docker compose -f docker/docker-compose.yml run --rm runner python scripts/run_cockpit_with_ib.py
```
...
```bash
docker compose -f docker/docker-compose.yml run --rm runner \
  python scripts/run_cockpit_with_ib.py --host host.docker.internal --port 7497   # ホストから見た API は 7497
```
...
IBKR_HOST=127.0.0.1 IBKR_PORT=7497 python scripts/telegram_cockpit_bot.py
```
