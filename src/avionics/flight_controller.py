"""
FlightController（管制層）：司令。Cockpit が返すスロットルモードを遷移・配布する。

役割: 「どのプロトコルを選ぶか」を決定し、対応する Protocol（作戦）を起動する。
Manual モード時は CockpitSignal を受信すると Telegram 承認を待ち、承認後に dispatch_protocol を実行する。
定義書「4-2 OS構造」「Phase 5 Telegram統合」参照。
"""
...
class FlightController:
    """
    管制層：Cockpit が返すスロットルモードを遷移・配布する。判定は Cockpit に委譲。
    ...
    def __init__(
        ...
        *,
        ...
        self._pending_approval_signal: Optional[CockpitSignal] = None
    ...
    def approval_granted(self) -> None:
        ...
        approval_event.set() により、承認待ち中の on_cockpit_signal が再開する。Phase 5 用。
        """
        self._approval_event.set()

    async def on_cockpit_signal(self, signal: CockpitSignal) -> None:
        """
        Cockpit からの信号を受信し、承認ゲートを経てプロトコルを起動する。
    ...
    async def request_telegram_approval(self, signal: CockpitSignal) -> None:
        """
    ...
    async def dispatch_protocol(self, signal: CockpitSignal) -> None:
        """
    ...
    async def pulse(self) -> None:
        """
        管制サイクル。Cockpit の三層方式で銘柄ごとにスロットルモード取得 → 遷移 → 全エンジンへ指令。
