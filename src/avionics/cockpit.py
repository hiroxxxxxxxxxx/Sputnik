"""
Cockpit（計器層）：三層制御（個別・同期・制限）の算出に特化。レイヤー混合を避ける。

個別制御層・同期制御層・制限制御層を算出し、get_effective_level(symbol) で実行レベルを返す。
CockpitSignal で「計器の結論」をカプセル化し、Flight Controller の承認ゲートへ渡す。
定義書「3-1 PFD」「4-2」「0-1-Ⅲ」参照。
"""
...
class CockpitSignal:
    """
    Cockpit が出力する「計器の結論」をカプセル化したデータ構造。
    Flight Controller の承認ゲート（Manual/Auto）や Protocol 起動の入力となる。
    定義書「Phase 5 Telegram統合」CockpitSignal 参照。
    """
                ...
                if isinstance(f, PFactor) and price is not None:
                    tasks.append(
                        f.update_from_signals(
                            daily_change=price.daily_change,
                            cum5_change=price.cum5_change,
                            downside_gap=price.downside_gap,
                            trend=price.trend,
                            cum2_change=price.cum2_change,
                        )
                    )
